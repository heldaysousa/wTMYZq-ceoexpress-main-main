...
import { PushSubscription } from '@remix-pwa/push'

const webPushPublicKey = import.meta.env.VITE_WEB_PUSH_PUBLIC_KEY

type Props = { onSubcribe?(subscription: PushSubscription): void }
...
const Toggle: React.FC<Props> = ({ onSubcribe }) => {
  ...
  const { mutateAsync: saveSubscription } = Api.push.onSubscribe.useMutation()
  const { mutateAsync: deleteSubscription } = Api.push.unsubscribe.useMutation({
    onError: (error) => {
      message.error(`Failed to unsubscribe: ${error.message}`)
    }
  })
    ...
    <>
      {isSupported && (
        <Flex vertical gap={20} className="py-10">
          <Typography.Title level={3}>Push Notifications</Typography.Title>

          {status === 'denied' ||
            (!canSendPush && (
              <Typography.Text type="secondary">
                Push notifications are currently disabled. You can enable them in your browser settings.
              </Typography.Text>
            ))}
          ...
          {status !== 'denied' && (
            <>
              <Flex justify="space-between" align="center">
                <Typography.Text>Enable Push Notifications</Typography.Text>
                <Flex gap={20}>
