...
const trpcRouter = Trpc.createRouter({
  onSubscribe: Trpc.procedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      const subscriptions = await ctx.database.subscription.findMany({
        where: { userId: user.id },
      })

      const subscriptionFound: PushSubscription = subscriptions
      ...
      if (subscriptionFound) {
        ...
        await ctx.database.subscription.create({
          data: {
  ...
  unsubscribe: Trpc.procedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      const subscriptionFound = subscriptions
        .map(item => ({
          ...item,
      ...
      if (subscriptionFound) {
        await ctx.database.subscription.delete({
  ...
  sendNotification: Trpc.procedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      if (input.userIds?.length > 0) {
        const items = await ctx.database.subscription.findMany({
      ...
      const pushSubscriptions = subscriptions.map(item => {
        const parsedContent = JSON.parse(item.content) as PushSubscription
        return {
